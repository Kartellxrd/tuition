...
export default function LearningScreen({navigation}){
 const[items,setItems]=useState([]),[sessions,setSessions]=useState([]),[loading,setLoading]=useState(true);
 const load=useCallback(()=>{setLoading(true);Promise.all([api.get("/enrollments/me"),api.get("/sessions/me/upcoming")]).then(([e,s])=>{setItems(e.data);setSessions(s.data)}).catch(()=>{}).finally(()=>setLoading(false))},[]);
...
const s=StyleSheet.create({
 ...
 pending:{flexDirection:"row",alignItems:"center",backgroundColor:"#111827",borderRadius:17,borderWidth:1,borderColor:"#3A3440",padding:16,marginBottom:10},pendingCode:{color:colors.white,fontWeight:"900"},pendingText:{color:"#FBBF24",fontSize:12,marginTop:5}
});