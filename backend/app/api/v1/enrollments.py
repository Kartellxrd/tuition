...
from app.schemas.enrollment import EnrollmentCreate, EnrollmentResponse, RejectEnrollmentRequest
...
def create_enrollment(payload: EnrollmentCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    price = module.group_price if payload.tier == EnrollmentTier.GROUP else module.one_on_one_price
    enrollment = Enrollment(student_id=user.id, module_id=module.id, tier=payload.tier, status=EnrollmentStatus.PENDING, expected_price=price, payment_reference=payload.payment_reference.strip(), proof_of_payment_storage_path=payload.proof_of_payment_storage_path)
